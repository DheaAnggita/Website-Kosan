...
$conn = mysqli_connect("localhost", "root", "", "kosan" );
...
function tambah($data){
    ...
    $result = mysqli_query($conn, "SELECT nomorkamar FROM kamarpa1 WHERE nomorkamar = '$nomorkamar'");
    if ( mysqli_fetch_assoc($result)) {
        echo "<script>
    ...
    //tambah user baru ke database
    mysqli_query($conn, "INSERT INTO kamarpa1 VALUES ('', '', '$nomorkamar', '$fasilitas', '$status', '$harga', '$foto' )");
...
function ubah($data){
	...
	$SQL =mysqli_query($conn, "UPDATE kamarpa1 SET 
		
		nomorkamar = '$nomorkamar',
		fasilitas = '$fasilitas',
        status = '$status',
        harga = '$harga',
        foto = '$foto' WHERE idkamar='$idkamar'");
        
       
       
        if($status=="Tersedia"){
            $booking=mysqli_query($conn, "DELETE FROM booking WHERE idkamar='$idkamar'");
...
function booking($data){
    ...
    mysqli_query($conn, "UPDATE kamarpa1 SET `status`='Tidak Tersedia', `iduser` = '$iduser' WHERE `idkamar`='$idkamar'");
...
function keluhan($data){
    ...
    //tambah user baru ke database
    mysqli_query($conn, "INSERT INTO keluhanpa1 VALUES ('', '$iduser', '$idkamar', '$isi')");
